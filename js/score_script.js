var dataset = [ {	Metric:	'Corn Yield'	,Year1:	0	,Year2:	0	,Year3:	0},
...
var Plot = function()
{
	...
	this.update = function() {
        ...
        plot.append("svg:path")
            .attr("d", function(){return arrowpath;})
            .style("fill", "url(#arrow-indicator-grad)")
            .style("opacity", "0.8");

        for(var i=1; i<4; i++) {
            plot.append("rect")
                .attr("x", function(){return coords[i];})
                .attr("y", 0)
                .attr("width", 100)
                .attr("height", 450)
                .style("fill", function() {return getPrecipFill(i);});

            plot.append("text")
                .attr("x", function(){return coords[i] + 50;})
                .attr("y", 100)
		...
		node.append("text")
		  .attr("class", function(d) {return "ia " + " metric-label " + d.Metric.replace(/ /g,'');})
		  .attr("fill", "black")
		  .attr("opacity", "0.3")
		  .attr("x", "440")
  		...
  		dataset.forEach(function(d,i) {
            ...
            if(denominator != 0) {legendrectvals.push(numerator/denominator);}
            else{legendrectvals.push(0)}
        }); // NEEDS FIXED
		...
		node.append("rect")
		  ...
		  .attr("width",function(d,i){return legendrectvals[i]/100*50})
		  .attr("class","ia")
		  .attr("height",12);

		node.append("rect")
		  .attr("fill", "gray")
		  .attr("x", function(d,i) {return 530+legendrectvals[i]/100*50;})
		  .attr("y", function(d,i) {return i*27+40})
		  .attr("width", function(d,i){return 50*(1-legendrectvals[i]/100)})
		  .attr("height",12);
		...
		*/
		...
		objs.forEach(function(obj,i){
		  ...
		  .append("circle")
		  .style("fill", "gray")
		  .attr("cx", function(d) {return xdata(obj) + 10;})
		  .attr("cy", function(d){return y(d[obj.replace(/ /g,'')]);})
              .attr("r", function(d) {})
              .style("opacity", 0.3)
		  .attr("id", function(d){return obj})
		  .attr("class", function(d) {return "ia " + d.Metric.replace(/ /g,'') +" " + obj.replace(/ /g,'');})
		...
		plot.selectAll("text.rule3")
			...
			.attr("class", "rule3")
			.attr("fill", "Black")
			.attr("x", function(d) {return xdata(d) + 10;})
			.attr("y", "75%")
			.attr("text-anchor", "middle")
		 ...
		 plot.selectAll(".rule3")
			.on("click", function(d) { 
				legendrectvals = [];
				dataset.forEach(function(e) {legendrectvals.push(e[d.replace(/ /g,'')]);});
				updateLegend();
		...
		plot.selectAll(".ia")  
		  .on("click", function(d) {
		  if(!MetricStack.some(function(e,i) {return d.Metric.replace(/ /g, '')==e})) {
	...
	/*  
	  ...
	  */
	  ...
	  MetricStack.forEach(function(metric, i) {
	    ...
	    plot.selectAll(".ia")
	      .filter(function (e) {return metric == e.Metric.replace(/ /g,'')})
	      .transition()
	      .duration(1000)
	      .style("opacity", 1)
